/**
 * ========================================================================
 * GT8 SURGIFLOW - RECYCLE BIN PATIENT RECOVERY UTILITY (SAFE-RESTORE)
 * ========================================================================
 * 
 * DESCRIPTION:
...
(async () => {
  console.log("%c--- Starting GT8 SurgiFlow Safe Recycle Bin Recovery ---", "color: #2563eb; font-weight: bold; font-size: 14px;");
  ...
  try {
    const activeRes = await fetch(`${siteUrl}/_api/web/lists/getbytitle('${listTitle}')/items?$select=Id,Status&$top=5000`, {
      headers: { "Accept": "application/json;odata=verbose" }
    });
  ...
  for (const item of recycleBinItems) {
    ...
    try {
      ...
      if (newlyAdded) {
        ...
        // D. Update its status immediately (Internal name: Status)
        const updatePayload = {
          "__metadata": { "type": listItemEntityType },
          "Status": "Discharged"
        };
