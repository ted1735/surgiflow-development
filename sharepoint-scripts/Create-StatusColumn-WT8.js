/**
 * ========================================================================
 * WT8 (CTPCU POC TOOL V5) - CREATE STATUS COLUMN
 * ========================================================================
 * 
 * DESCRIPTION:
...
(async () => {
  console.log("%c--- Creating Status Column on WT8 List ---", "color: #2563eb; font-weight: bold; font-size: 14px;");
  ...
  try {
    ...
    if (createRes.ok) {
      ...
      const msg = errData.error ? errData.error.message.value : "Creation warning/error";
      
      if (msg.includes("already exists")) {
        console.warn(`Column "${fieldName}" already exists. Proceeding to update...`);
      } else {
        console.warn(`Column creation response: ${msg}. Proceeding to configure...`);
      }
    }
